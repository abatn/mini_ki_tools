...
exports.getCurrentFileInfo = getCurrentFileInfo;
...
var SetupState;
(function (SetupState) {
    SetupState[SetupState["Idle"] = 0] = "Idle";
    SetupState[SetupState["Checking"] = 1] = "Checking";
    SetupState[SetupState["CreatingVenv"] = 2] = "CreatingVenv";
    SetupState[SetupState["Installing"] = 3] = "Installing";
    SetupState[SetupState["Starting"] = 4] = "Starting";
    SetupState[SetupState["Ready"] = 5] = "Ready";
    SetupState[SetupState["Error"] = 6] = "Error";
})(SetupState || (SetupState = {}));
let currentSetupState = SetupState.Idle;
function updateSetupStatus(state, message) {
...
async function runAutoSetup(context) {
    ...
    if (!venvExists) {
        ...
        try {
            ...
            const createResult = await new Promise((resolve) => {
                const createProcess = (0, child_process_1.spawn)('python3', ['-m', 'venv', venvPath], {
    ...
    try {
        const installResult = await new Promise((resolve) => {
            const pipProcess = (0, child_process_1.spawn)(venvPython, ['-m', 'pip', 'install', '-r', requirementsPath], {
                stdio: ['pipe', 'pipe', 'pipe']
            });
            let stdout = '';
            let stderr = '';
...
function startPythonSubprocess(context, pythonPath) {
    ...
    try {
        pythonProcess = (0, child_process_1.spawn)(pythonPath, [extensionPythonPath, '--workspace', envWorkspacePath], {
            env,
            stdio: ['pipe', 'pipe', 'pipe']
        });
        let buffer = '';
...
async function askUserPermission(toolName, description) {
    return new Promise((resolve) => {
        ...
        vscode.window.showInformationMessage(`Mini KI: Erlaube '${toolName}'? ${description}`, { modal: false }, 'Erlauben', 'Ablehnen').then((selection) => {
...
function loadWorkspaceRules(workspacePath) {
...
async function activate(context) {
    ...
    const allowPermissionCmd = vscode.commands.registerCommand('mini-ki-tools.allowPermission', async (toolName) => {
    ...
    const denyPermissionCmd = vscode.commands.registerCommand('mini-ki-tools.denyPermission', async (toolName) => {
        const pending = pendingPermissions.find(p => p.tool === toolName);
        if (pending) {
            pending.resolve(false);
            pendingPermissions = pendingPermissions.filter(p => p.tool !== toolName);
...
async function startAgent() {
    try {
        ...
        const response = await axios_1.default.get(`${serverUrl}/health`, { timeout: 5000 });
        state.connected = true;
...
async function openChatPanel() {
    ...
    if (panel) {
        ...
        panel = vscode.window.createWebviewPanel('mini-ki-tools-panel', 'Mini KI Tools', vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    if (editor) {
        ...
        panel.webview.postMessage({
            ...
            selection: selection.isEmpty ? undefined : content.substring(selection.start.character, selection.end.character)
        });
    }
}
...
async function analyzeCode() {
    ...
    try {
        const response = await axios_1.default.post(`${serverUrl}/chat`, {
            messages: [{
